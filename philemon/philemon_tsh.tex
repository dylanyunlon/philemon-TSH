...
\label{sec:system}
% =============================================================================
% --- Claude #2 fills this section ---
% Template analogue: DES-LOC Section 2 (DES-LOC Algorithm).
% Required structure:
%   \subsection{Separation of Access Timescales}   (analogue of DES-LOC 2.1
%       "Separation of Optimizer-State Timescales") -- motivate why hot/dense
%       intervals belong in HBM and cold/sparse in DRAM; the access-frequency
%       and temporal-density signals the MigrationScheduler uses; "from C, the
%       good example" lineage: RapidStore snapshot_edges -> TieredAllocator.
%   \subsection{The Philemon-TSH Architecture}  (analogue of DES-LOC 2.2
%       "DES-LOC Algorithm") -- TieredAllocator waterfall, TemporalBridge
%       ingest/partition/query, MigrationScheduler sweep, SeqLock read path,
%       SlabAllocator. Present the ingest+query pipeline as Algorithm 1
%       (\begin{algorithm}...\end{algorithm}), mirroring DES-LOC's Algorithm 1.
% Grounding: src/core/tiered_allocator.hpp, src/bridge/temporal_bridge.hpp,
%   src/scheduler/migration_scheduler.hpp, src/core/seqlock.hpp,
%   src/core/slab_allocator.hpp, src/core/interval_index.hpp,
%   src/core/partition_skiplist.hpp; REVIEW_M005_M006.md, REVIEW_M007_M008.md.
\todoclaude{Claude \#2: The Philemon-TSH System (\S\ref{sec:system}).}

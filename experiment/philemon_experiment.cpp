/**
 * philemon_experiment.cpp — 全流程实验入口
 *
 * 策略: mv upstream 骨架 + 20% 动态修改 + 断点调试密布
 *
 * 来源文件 (upstream/rapidstore/) → 移植后在此文件统一编译:
 *   graph/edge.cpp          (38行)  → WeightedEdge         [MOD: 加tier_id字段]
 *   graph/edgeStream.cpp    (82行)  → TieredEdgeStream      [MOD: load时打印进度]
 *   readers/reader.cpp      (17行)  → ReaderFactory          [KEEP ~100%]
 *   readers/edgeListReader  (76行)  → EdgeListReader         [MOD: 带计数的read]
 *   readers/vertexReader    (59行)  → VertexReader           [MOD: 带计数的read]
 *   algorithms/BFS.cpp      (302行) → TieredBFS              [MOD: tier感知+debug dump]
 *   algorithms/pageRank.cpp (159行) → TieredPageRank         [MOD: 收敛监控]
 *   algorithms/SSSP.cpp     (175行) → TieredSSSP             [MOD: bin分布追踪]
 *   algorithms/WCC.cpp      (137行) → TieredWCC              [MOD: 组件合并追踪]
 *   utils/commandLineParser (700行) → ConfigEngine           [MOD: 简化为直接解析]
 *   types/types.hpp         (150行) → 统一类型               [MOD: 加philemon扩展]
 *   main.cpp                (202行) → 本文件main()           [MOD: tier循环+debug]
 *
 * 修改摘要 (每个模块 ~20% 变更):
 *   1. namespace: driver::* → philemon::experiment::*
 *   2. 接口: m_interface/m_snapshot → 模板化FakeGraph/FakeSnapshot
 *   3. debug: 每个关键循环加 BREAKPOINT_DUMP() 打印当前全部结构体状态
 *   4. tier: 算法内部加 tier_hit_count[3] 追踪 DRAM/CXL/SSD 访问
 *   5. 收敛: PageRank加delta收敛追踪, SSSP加bin分布, WCC加组件大小
 *
 * Build:
 *   g++ -std=c++17 -O2 -pthread -o philemon_experiment philemon_experiment.cpp
 *
 * Run:
 *   ./philemon_experiment [config.cfg]   # 或无参数用合成数据
 */
...
namespace philemon { namespace experiment {
...
#define BREAKPOINT_DUMP(tag, ...) do { \
    if (philemon::experiment::g_debug_level >= 2) { \
        std::printf("[BP·%s] ", tag); \
        std::printf(__VA_ARGS__); \
        std::printf(" | RSS=%ld KB\n", philemon::experiment::get_rss_kb()); \
    } \
} while(0)

#define PROGRESS_PRINT(phase, cur, total) do { \
    if (philemon::experiment::g_debug_level >= 1 && (cur) % std::max(1UL,(total)/20) == 0) { \
...
int main(int argc, char** argv) {
    ...
    // ─── Banner ─────────────────────────────────────────────────────
    std::printf("╔═══════════════════════════════════════════════════════════╗\n");
    std::printf("║  Philemon-TSH Experiment Runner                          ║\n");

...
public:
    // Constructor
    explicit Neo_Graph_Wrapper(bool is_directed = false, bool is_weighted = true, int block_size = 1024)
    ...
    bool run_batch_edge_update(std::vector<std::pair<uint64_t, uint64_t>> &edges, int start, int end);
    ...
    class Snapshot {
    private:
//        const TransactionManager<NeoGraphContainer> &m_tm;
//        ReadTransaction<NeoGraphContainer> m_transaction;
    ...
    public:
        explicit Snapshot(const TransactionManager &tm) :
                                                                             ...
                                                                             snapshot{tm.index_impl, tm.write_timestamp, true} {}
        ...
        [[nodiscard]] uint64_t logical2physical(uint64_t logical) const;
        ...
        void edges(uint64_t index, std::vector<uint64_t> &) const;
    ...
    static std::string repl() {
        return std::string{"Neo_Graph_Wrapper"};
    }
};